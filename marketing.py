...
function initCalculatorAutocomplete() {
  ...
  setTimeout(function() {
    ...
    // Hide custom dropdown elements (no longer needed)
    const acFrom = document.getElementById('ac_from');
    const acTo = document.getElementById('ac_to');
    if (acFrom) acFrom.style.display = 'none';
    if (acTo) acTo.style.display = 'none';
    
    // Use native Google Places Autocomplete (same as landing page)
    function initGooglePlacesAutocomplete() {
      if (typeof google === 'undefined' || !google.maps || !google.maps.places) {
        // Retry after a short delay if Google Maps not loaded yet
      ...
      if (fromInput) {
        ...
        fromAutocomplete.addListener('place_changed', function() {
          ...
          if (place.place_id) {
            const placeIdInput = document.getElementById('from_place_id');
      ...
      if (toInput) {
        ...
        toAutocomplete.addListener('place_changed', function() {
          ...
          if (place.place_id) {
            const placeIdInput = document.getElementById('to_place_id');
            if (placeIdInput) placeIdInput.value = place.place_id;
